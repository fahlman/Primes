...
final class PrimeSieve {
    ...
    func runSieve() {
        ...
        while p <= limit / p {
            ...
            if bytes[candidate >> 3] & (UInt8(1) << (candidate & 7)) == 0 {
                ...
                // Factors above 79 leave at most one multiple in each 64-bit word.
    ...
    /// Fused byte marking for odd factors above 79, from bit `start` onward. Each
    ...
    /// Dense marking on 128-bit chunks for every odd factor from 65 through 79.
    ...
    private func markVectorDenseMultiples(of p: Int) {
        ...
        default:
            preconditionFailure("Vector-dense marking requires an odd factor from 65 to 79")
        }
