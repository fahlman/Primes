...
def api(endpoint):
    # Structured argv only. gh follows the API's artifact/job-log redirects.
...
def main():
    ...
    destination = EVIDENCE / args.architecture
    ...
    destination.mkdir()
    collection = {"started_at_utc": utc(), "architecture": args.architecture, "source_revision": SOURCE,
    ...
    try:
        ...
        write_new(github / "job.log", api(f"repos/{REPOSITORY}/actions/jobs/{job['id']}/logs"))
