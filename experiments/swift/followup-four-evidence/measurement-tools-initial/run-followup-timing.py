...
assert len(sys.argv) == 2 and sys.argv[1] in sessions, 'Specify one registered session'
session = sys.argv[1]
...
out = pkg / (session + '-results-20260912.json')
record = pkg / (session + '-execution-20260912.json')
compiler_log = pkg / (session + '-compiler-20260912.jsonl')
assert all(not p.exists() for p in (out, record, compiler_log)), 'Preserve existing evidence'
entry = {'session': session, 'started_at_utc': utc(), 'review_record_sha256': sha(review_path),
