...
record = {'candidate': a.candidate, 'control': control, 'harness': harness,
          ...
          'run_support_sha256': hashlib.sha256((Path(__file__).resolve().parents[1]/'primes_run_support.py').read_bytes()).hexdigest(),
...
try:
    ...
    boundary = b'    /// Dense marking on 128-bit chunks'
    assert source.split(boundary)[0] == control_source.split(boundary)[0], 'Changes before the 128-bit path'
    assert source[source.index(b'    func primes()'):]==control_source[control_source.index(b'    func primes()'):], 'Changed enumeration/storage API'
    for path in ['Benchmark.swift','BenchmarkObserver.swift','Verify.swift','ExtraVerify.swift','tools/phase-split/PhaseSieve.swift','tools/phase-split/PhaseVerify.swift','tools/generate-dense-128.swift','compare_optimizations.py']:
        assert git('show', f'{a.candidate}:experiments/swift/{path}') == git('show', f'{control}:experiments/swift/{path}'), f'Changed preserved file {path}'
    record['preserved_prefix_suffix_and_reference_files_equal'] = True
    ...
    with TimingLock(f'Codex {a.tag} source/codegen/correctness {a.candidate}'):
        ...
        record['generator_check'] = 'Not applicable: the 128-bit generated block is removed; legacy generator is retained unchanged as historical tooling. Unchanged 64-bit source is asserted separately.'
        for name, files in [
            ...
            ('recurrence', ['PrimeSieve.swift','tools/phase-split/PhaseSieve.swift','RecurrenceVerify.swift'])
        ]:
