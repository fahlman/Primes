// Check every valid odd flag against an independent Boolean sieve stopped at the
// same factor. Also compare copied/full buffers byte-for-byte, including padding.
import Foundation
...
struct PhaseVerify {
    static func main() {
        ...
        for limit in limits.sorted() {
                        ...
                        production.withStorage { expected in
                            sieve.withStorage { actual in
                                precondition(memcmp(expected!, actual!, byteCount) == 0,
                                             "Full copy mismatch at limit \(limit)")
                            }
                        }
                    }
            ...
            let allocated = PhaseSieve(limit: limit)
        ...
        print("Passed \(checks) partial/full flag checks over \(limits.count) limits; allocation zeroing; full raw-buffer equality; 78,498 primes at 1,000,000.")
    }
}
