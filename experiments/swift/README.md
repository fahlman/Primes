...
Edit the handwritten sieve in `tools/PrimeSieve.swift.in`; the generator inserts both switches and derives the runtime dispatch bounds from their factor ranges. `PrimeSieve.swift` remains the committed, directly compiled source, so builds do not require generation. `--check` compares the **complete file**, including handwritten code and formatting. `--write` renders the **complete file** with one atomic write after validating the template and both existing marker pairs. Manual changes anywhere in the rendered file are replaced; make those changes in the template instead. Both commands reject missing, duplicated, reversed or overlapping source markers. The template also checks the supported runtime dispatch shape, contiguous odd ranges and helper widths; changing that mechanism requires reviewing the generator contract.

With no arguments, the generator still prints both marked blocks. `--template PATH` may appear before or after the check/write pair to select a disposable template fixture; otherwise the template beside the generator is used, independently of the working directory. The old `tools/generate-dense-128.swift` path forwards the complete interface. Historical generators, recorded tool hashes and evidence remain tied to their original commits.

Run `swift tools/check-dense-generator.swift` under the same lock for reusable generator checks. It checks every generated factor's individual-mark schedule independently, full-file identity, canonical and compatibility entry points, repair behavior, malformed templates/markers/arguments, and preservation on failed writes. All edits use disposable fixtures; the final JSON records each subprocess result. This is an authoring-tool check, not a sieve benchmark.

The `swift/dense-generator` refactor adds only the two 64-bit marker comments to `PrimeSieve.swift`. Removing those lines reproduces adopted development `1d05221` byte for byte; the explicit calls, runtime helpers, alignment peels, tails, allocation, runner, observer, and verifiers are unchanged. Source comparisons cover both generated blocks. Generator execution and error-path checks passed. The complete optimized sieve-plus-benchmark assembly is byte-identical to the control, so no throughput timing was needed. See [the generator review](reports/DenseGeneratorReview.md) for exact revisions and evidence.
