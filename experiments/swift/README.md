...
## Square-root discovery bound

Branch `swift/discovery-bound` is stacked on unmerged `bd3858c` and changes only the factor-discovery loop's condition. `p <= limit / p`, a division on every odd candidate (about 500 per pass at one million), becomes `p <= sqrtLimit`, where `sqrtLimit` is the integer square root of the limit, computed once per pass inside `runSieve` so it stays inside the timed pass. Both conditions hold exactly when p² ≤ limit: the floating-point estimate is corrected to the exact root with the same division, and a limit below 1 gives a root of 0, so no candidate is tested, as before. Stopping at √limit is explicitly allowed. An earlier cached-square-root test found no reliable gain when a pass took about 120 µs; this retests the idea at about 39 µs, where the same saving is three times the share.
