...
With no arguments, the generator still prints both marked blocks. `--template PATH` may appear before or after the check/write pair to select a disposable template fixture; otherwise the template beside the generator is used, independently of the working directory. Historical generators, recorded tool hashes and evidence remain tied to their original commits.

Run `swift tools/check-dense-generator.swift` under the same lock for reusable generator checks. It checks every generated factor's individual-mark schedule independently, full-file identity, repair behavior, malformed templates/markers/arguments, and preservation on failed writes. All edits use disposable fixtures; the final JSON records each subprocess result. This is an authoring-tool check, not a sieve benchmark.
