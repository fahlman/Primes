...
Branch `swift/atomic-marks` is stacked on unmerged `bd3858c` and changes only how the sparse loop's marks are written. The buffer is still allocated and zeroed as bytes; for each factor above 127, `runSieve` views it as `Atomic<UInt8>` through `withMemoryRebound`, which changes no bytes, and `markSparseMultiples` marks each multiple with `bitwiseOr(mask, ordering: .relaxed)`. On arm64 that is one `ldsetb` instruction per mark, an atomic read-modify-write in the memory system, instead of the `ldrb`, `orr`, `strb` sequence. The loop structure, offsets, masks, cleanup group and tail are unchanged, so the comparison isolates the marking instruction. `Synchronization` is part of the Swift 6 toolchain, not an external dependency. The sieve remains single-threaded; relaxed ordering asks for nothing beyond the read-modify-write itself. Each composite is still marked by its own operation in source, now also by its own instruction in machine code. No prediction is recorded here; the timing session decides. Atomic instructions behave differently on x86-64 (`lock or`), which only a Linux/amd64 measurement can show.
...
The Dockerfile uses official `swift:6.3.3` and `swift:6.3.3-slim` images. The image tags were checked for amd64 and arm64. Docker is not installed on the test Mac; separate Linux/Docker validation is pending, with no completed compatibility claim here.
