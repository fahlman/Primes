...
- Refactor maintenance was adopted in order through [PR #26](https://github.com/fahlman/Primes/pull/26), merge `0d64a47`, and [PR #27](https://github.com/fahlman/Primes/pull/27), merge `b33b2fc`. The authoring template, generator contract and 98 reusable checks accompany shared storage sizing at tested source `5cbc25e`. Complete M4 Pro / Swift 6.3.3 benchmark assembly remains byte-identical to control `84d8a4f`; Verify ASan/WMO, ExtraVerify ASan and both integrated generator checks passed. These are maintenance changes, with no new throughput result. [Authoring report](reports/AuthoringTemplateReview.md), [storage report](reports/StorageLayoutReview.md), [merge identities](refactor-merge-evidence/2026-09-13.json).
- [PR #28](https://github.com/fahlman/Primes/pull/28), exact package `b826fc6`, was adopted into the fork's `swift/submission-review-base` through merge `35becbe`. The local prepared submission was fast-forwarded to that merge. It includes the shared duration conversion, exclusive result slice and the same reviewed core/template. Release/ASan package checks, 28 CLI cases and 19 duration probes passed; [package report](https://github.com/fahlman/Primes/blob/3fb586b/experiments/swift/reports/SubmissionCLIRefactor.md). Upstream submission remains paused; native Linux/Docker package validation and a fresh direct upstream timing comparison remain separate submission work.
- The current-source phase tooling in [PR #24](https://github.com/fahlman/Primes/pull/24), exact `0c605a4`, passed ASan/WMO (66,845 checks over 2,305 limits each), source/overwrite guards and independent actual-binary review. It was adopted via merge commit `2ee7b3a`. Fresh integration generation and all eight short wrapper checks passed after PR #21; [integration record](maintenance-integration-evidence/checks-2ee7b3a.json). [Report](reports/CurrentPhaseTools.md). Production marking is unchanged; no new phase timing campaign ran.
- `swift/dense-small-factors` retains cutoff 111, exact historical measured source `099e35a`, adopted through [PR #16](https://github.com/fahlman/Primes/pull/16) in merge commit `380a942`. It incorporates PR #14's 128-bit marking and proven-safe wrapping byte offsets; the earlier 127 cutoff is superseded. That adoption preserved the reviewed source bytes; subsequent maintenance is recorded above. The recorded throughput measurements remain specific to their measured revisions.
...
- The fork-only Linux/Docker workflow was adopted through [PR #15](https://github.com/fahlman/Primes/pull/15), merge commit `f639c15`, after fixing owned-container cleanup on timeout/cancellation. At workflow `0281255`, one serial native amd64/arm64 run passed all six Swift checks, runtime smoke, 13 lifecycle tests and four real-Docker probes per architecture. All 14 validation containers and four probe containers were confirmed removed before lock release. Exact tested solution remains adopted `099e35afa8a2f01d79ef11f805d760e81d1d983a`; integration leaves the current sieve and benchmark unchanged. [Cleanup report and permanent evidence](reports/LinuxContainerCleanup.md); [earlier exact-cutoff report](reports/Cutoff111LinuxValidation.md). These are correctness and compatibility results, not Linux or Threadripper performance.
...
- Future [phase diagnostics](tools/phase-split/README.md) derive their source from current `PrimeSieve.swift` with three checked substitutions and reject changed source identities; the runner includes a through-111 stage. The unchanged `PhaseSieve.swift` and `PhaseVerify.swift` remain the historical B (`8f108f5`) correctness reference. The [seven-mode B breakdown](reports/FusedSparseBandBreakdown.md) measured copied full at 39.962 µs versus production 40.183 µs, with sparse factors accounting for approximately 67.5%. Those historical figures do not profile adopted cutoff 111; the tooling update starts no new phase-timing campaign.
