...
- User-requested PR #20 repeat, after reported concurrent computer use in the first session: same candidate measured 366.526 µs versus same control at 38.489 µs; all six runs valid and every candidate trial slower. Retain rejection. The post-run monitoring wrapper failed on compiler process-group identification during compilation; timed-run samples contained only the owned benchmark, with backup idle and no audio. [Separate report and preserved caveat](reports/CompactDenseLoopsRetest.md).
...
| Compact dense loops: user-requested repeat | [PR #20](https://github.com/fahlman/Primes/pull/20), unchanged `1a259a8`; [retest](reports/CompactDenseLoopsRetest.md) | 0.366526 | Every candidate trial slower than control; retain rejection; monitoring-process caveat preserved |
