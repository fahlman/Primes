...
- The most recent direct upstream comparison measured the earlier `e8ba734`: 0.059299 ms versus 0.207650 ms for upstream striped UInt8, **3.50x throughput**. [Report](reports/UpstreamBaselineComparison.md), [raw results](upstream-baseline-e8ba734.json). It identifies the upstream baseline but does not measure the current candidate's upstream advantage; do not combine ratios across sessions. A refresh is authorized and pending.
...
| Sixteen-write fused sparse loop | [PR #12](https://github.com/fahlman/Primes/pull/12), `59262fe`; [review](reports/SixteenWriteFusedReview.md) | 0.039553 | Adopted in `83751ea`; current best |
