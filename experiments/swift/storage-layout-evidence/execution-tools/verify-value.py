...
BUILD = ROOT / '.build/storage-layout-5cbc25e'
EVIDENCE = ROOT / 'storage-layout-evidence/5cbc25e'
...
def main():
    ...
    try:
        ...
        with TimingLock('Codex storage-layout 5cbc25e serial assembly and correctness verification'):
            ...
            try:
                ...
                save()
                record['additional_candidate_functions'] = sorted(set(parsed['candidate']) - set(parsed['control']))
                if not all(parsed['candidate'].get(name) == body for name, body in parsed['control'].items()):
                    raise RuntimeError('Assembly differs; stop before final-source correctness and report for review')
                record['existing_function_identity_gate_passed'] = True
                save()
