...
The package prepared through [PR #28](https://github.com/fahlman/Primes/pull/28), preserved at `35becbe`, is now the development source under `PrimeSwift/solution_1`. The entry is submitted as [upstream PR #1083](https://github.com/PlummersSoftwareLLC/Primes/pull/1083) from `swift/submission` at `0222520`. It prints the label `yellowcub_fahlman_striped_UInt8`, following the folder's `j-f1_yellowcub_bool` precedent for an improved entry. The packaged entry's own image is what [the Linux workflow](linux-docker/README.md) validates.
