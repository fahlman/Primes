...
- Use https://github.com/fahlman/Tetris/issues/41 as the canonical machine reservation registry shared with Tetris and Euchre. Follow the shared guide's reservation and release rules for benchmark runs. Before a build, test, or benchmark that could interfere with timing, read the registry's body and comments and check running and queued jobs, including the EuchreKitBots post-merge exam. Ordinary builds and tests need no reservation of their own, but must respect existing reservations.
...
Run these from `fork/swift`, holding the timing lock. Set `swift_package=../../PrimeSwift/solution_1/PrimeSwift_1bitStriped_u8` in the shell first. A candidate is ready for timing only when all of them pass.
